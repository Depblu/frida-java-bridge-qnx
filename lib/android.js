...
function getAndroidSystemProperty (name) {
  if (systemPropertyGet === null) {
    systemPropertyGet = new NativeFunction(Module.getExportByName('libc.so.5', '__system_property_get'), 'int', ['pointer', 'pointer'], nativeFunctionOptions);
  }
...
`, {
    ...
    strtoul: Module.getExportByName('libc.so.5', 'strtoul')
  });
...
function makeSocketPair () {
  if (socketpair === null) {
    socketpair = new NativeFunction(
      Module.getExportByName('libc.so.5', 'socketpair'),
      'int',
      ['int', 'int', 'int', 'pointer']);
  }
